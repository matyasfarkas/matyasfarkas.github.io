...
\paragraph{What you find.} The preset is deliberately minimal: a long-run real-growth
rate for the economy, an inflation anchor where the central bank has a clear public
target, and --- for the economies that carry a full public-finance block --- long-run
values for revenue, primary spending, and the effective interest rate on debt. It does
not set policy rates, unemployment, or asset prices. Japan ships with no preset at
all --- its long run is left entirely model-driven, so the block simply does not appear
there. The preset is \emph{off} by default everywhere: until you click the button, you
are looking at the raw model.
...
\emph{bare minimum} (long-run real growth and one inflation anchor per
economy, plus the effective-rate/revenue/expenditure trio for the six
fiscal-block economies) so the joint system stays well conditioned out of
the box. Well-conditioned is not the same as well-shaped, however: because
