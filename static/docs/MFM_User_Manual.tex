...
\textbf{Joint}. \textbf{Joint} is the default.
...
\item \textbf{Joint} (default) --- the full $M\,\delta c = \psi^\star -
\psi^{\,0}$ system, so \emph{every} anchored terminal lands exactly on its
value. \emph{Correct}, but a strongly-correlated target set makes $M$
ill-conditioned and can \emph{destabilise} the path; the tool flags that
case and the user switches to per-variable. A genuinely singular $M$ falls
back to the per-variable $\delta c$ numerically (never a NaN path).
\end{itemize}
...
fiscal-block economies) precisely so the joint system stays well
conditioned out of the box: under these defaults joint is exact and stable
on essentially every bloc, so per-variable is offered as the deliberate
stability escape when a user adds many correlated targets.
